...
面向 Windows 的本地优先中文语音助手。目前是 **Alpha 源码版**：能够唤醒、识别、通过本地 Ollama 模型回答问题，并使用受控工具打开应用、查询英超比分和设置真正会触发的提醒。它还不是安装即用的稳定版。

## 功能

- 中文唤醒词“贾维斯”、本地 Whisper 语音识别、Ollama 对话、Windows 中文语音播报。
- 全屏科技 HUD，鼠标与键盘输入可穿透到桌面；`F8` 显示/隐藏 HUD，`F9` 免唤醒词对话。
...
3. 双击 `Start Jarvis.bat`，说“贾维斯”，听到提示音后说出任务。首次加载 Whisper 可能较慢。
...
运行时会在本机 `data/jarvis_memory.db` 中记录对话、提醒、工具操作、前台应用名和窗口标题。窗口标题可能含有个人信息；该数据库和临时录音均被 `.gitignore` 排除，不应上传。若不想保留历史，可以在退出程序后自行备份或删除 `data/`；当前尚无界面内的隐私管理开关。
...
- 模型名、唤醒灵敏度和麦克风阈值仍在代码中配置，设置界面及安装包是后续工作。
